...
class DisplayManager:
    ...
    @staticmethod
    def display_status(mode_title: str, status_message: str, mapsheets: List['MonitorMapSheet']):
        """显示监控状态信息"""
        ...
        # 显示监控表格
        if mapsheets:
            headers = ["TEAM", "NAME", "PERSON", "STATUS", "FILENAME"]
            table_data = []
            
            for mapsheet in mapsheets:
                status = "已收集" if mapsheet.finished else "等待中"
                filename = mapsheet.mapsheetFileName if mapsheet.mapsheetFileName else "-"
