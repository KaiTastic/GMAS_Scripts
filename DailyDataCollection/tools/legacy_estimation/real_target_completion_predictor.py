...
def get_daily_progress_rate(mapsheet_id, days=7):
    ...
    try:
        ...
        return max(10, daily_rate)  # 最小每�?0�?
        
    except Exception as e:
        print(f"⚠️ 计算日进度失�?({mapsheet_id}): {e}")
        return 50  # 默认�?


def predict_completion_date_with_real_targets():
    """使用真实目标点数预测完成日期"""
    print("🎯 基于真实目标点数的图幅完成日期预�?)
    print("=" * 80)
    ...
    try:
        ...
        for mapsheet_id, meta in metadata.items():
            ...
            days_str = f"{days_remaining:3d}�? if days_remaining < 999 else "   --"
            
            print(f"{mapsheet_id[:20]:20s} | {progress_str:12s} | {days_str:8s} | "
                  f"{estimated_date:14s} | {status}")
        
        print("-" * 80)
        print(f"完成状�? {completed_count}/{len(metadata)} 已完成，{len(metadata) - completed_count} 进行�?)
        ...
        if completion_predictions:
            ...
            print(f"\n📊 完成预测分析:")
            print(f"  最早完�? {earliest['mapsheet_id']} ({earliest['estimated_date'].strftime('%Y-%m-%d')})")
            print(f"  最晚完�? {latest['mapsheet_id']} ({latest['estimated_date'].strftime('%Y-%m-%d')})")
            ...
            if total_remaining > 0:
                ...
                print(f"  剩余总点�? {total_remaining:,}")
                print(f"  平均日完成率: {avg_daily_rate:.1f} �?�?)
    ...
    try:
        ...
        for mapsheet_id, meta in metadata.items():
            ...
            if completion_rate >= 1.0:
                progress_groups['已完�?(100%)'].append(mapsheet_id)
            elif completion_rate >= 0.8:
                progress_groups['接近完成 (80-99%)'].append(mapsheet_id)
            elif completion_rate >= 0.6:
                progress_groups['大部分完�?(60-79%)'].append(mapsheet_id)
            elif completion_rate >= 0.4:
                progress_groups['一半完�?(40-59%)'].append(mapsheet_id)
            elif completion_rate >= 0.2:
                progress_groups['开始阶�?(20-39%)'].append(mapsheet_id)
            else:
                progress_groups['刚开�?(0-19%)'].append(mapsheet_id)
        
        # 显示分组结果
        for group_name, mapsheets in progress_groups.items():
            if mapsheets:
                print(f"{group_name}: {len(mapsheets)}�?)
        ...
        completed_count = len(progress_groups['已完�?(100%)'])
        near_completion_count = len(progress_groups['接近完成 (80-99%)'])
        
        print(f"总结:")
        print(f"  总图幅数: {total_count}")
        print(f"  已完�? {completed_count} ({completed_count/total_count:.1%})")
        print(f"  接近完成: {near_completion_count} ({near_completion_count/total_count:.1%})")
        print(f"  待完�? {total_count - completed_count} ({(total_count - completed_count)/total_count:.1%})")
...
def generate_summary_report():
    ...
    try:
        ...
        # 显示基本统计
        print(f"📊 基本统计:")
        print(f"  项目范围: {summary.get('total_mapsheets', 0)} 个图�?)
        print(f"  当前进度: {summary.get('total_points', 0):,} / {summary.get('total_target_points', 0):,} 观测�?)
        print(f"  完成�? {summary.get('overall_completion_rate', 0):.1f}%")
        print(f"  已完成图�? {summary.get('completed_mapsheets', 0)} �?)
        print(f"  进行中图�? {summary.get('in_progress_mapsheets', 0)} �?)
        
        # 计算预期完成时间
        remaining_points = summary.get('total_target_points', 0) - summary.get('total_points', 0)
        if remaining_points > 0:
            # 估算日均完成速度（可以基于历史数据优化）
            estimated_daily_rate = 200  # 假设团队每天能完�?00�?
            days_to_completion = remaining_points / estimated_daily_rate
            completion_date = datetime.now() + timedelta(days=days_to_completion)
            
            print(f"\n⏱️ 预期完成时间:")
            print(f"  剩余工作�? {remaining_points:,} 观测�?)
            print(f"  预估完成日期: {completion_date.strftime('%Y�?m�?d�?)}")
            print(f"  剩余天数: {int(days_to_completion)} �?)
...
def main():
    ...
    try:
        ...
        if predictions:
            ...
            print(f"  �?使用真实�?Adjusted Num'目标点数")
            print(f"  �?基于历史数据计算日均进度")
            print(f"\n📝 建议:")
            print(f"  �?重点关注进度较慢的图�?)
            print(f"  �?定期更新Excel数据以获得更准确的预�?)
